using Stack_Hanoi_tower;

namespace Stack_Hanoi_Tower
{
    public class Program
    {
        private const int DISCS_COUNT = 8;
        ...
        private static void Algorithm_Visualize(object sender, EventArgs e)
        {
            ...
            algorithm.Auxiliary);
            Console.WriteLine(Center("Source:") + Center("Destination:") +
            Center("Auxiliary:"));
            DrawVisualization(visualization);
            Console.WriteLine();
            Console.WriteLine($"Number of moves: {algorithm.MovesCount}");
            Console.WriteLine($"Number of disks : {algorithm.DiscsCount}");
